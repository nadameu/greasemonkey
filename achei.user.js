...
async function getDominio(doc) {
    var _a, _b, _c;
    const dominio = (_c = (_b = (_a = doc
        .querySelector('[name="local"]:checked')) === null || _a === void 0 ? void 0 : _a.nextSibling) === null || _b === void 0 ? void 0 : _b.textContent) === null || _c === void 0 ? void 0 : _c.trim().toLowerCase();
    if (dominio == null)
        throw new Error('Domínio não encontrado.');
    return dominio;
}
async function getNodeInfo(formulario) {
    const info = Array.from((function* () {
        for (const sibling of siblings(formulario))
            for (const node of flattenTabela(sibling))
                yield* getNodeSigla(node);
    })());
    if (info.length === 0)
        throw new Error('Nenhuma sigla encontrada.');
    return info;
}
