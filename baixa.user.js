...
function fromId(id) {
	...
	return null === element
		? null
		: (element =>
				element.matches('input[type="radio"][name]') && '' !== element.name
					? element
					: null)(element);
}
		...
		(({ baixar, pendencias, wrapper, sidebar }) => {
			...
			const osd = ((mensagemInicial, doc = document, win = doc.defaultView) => {
					...
					function onFimTransição() {
						if (!mensagemInicialMostrada) {
							...
							win.addEventListener(
								'resize',
								debounce(150, ajustarTamanhoFonte)
							);
							ajustarTamanhoFonte();
						}
					}
					function alteraTextoAguardaEFade(texto) {
						...
						div.getBoundingClientRect();
						div.style.transition = `opacity 500ms linear ${((mensagem = texto),
						Math.max(
							ESPERA_MINIMA,
					...
					function ajustarTamanhoFonte() {
						div.style.fontSize = `${(doc.documentElement.clientHeight / 3) |
							0}px`;
					}
				})(
							...
							return (grupos => {
								const { elementos, maximo } = (grupos => {
									const { grupos: radiosComValor, maximo } = (grupos => {
										const quantidades = grupos.map(grupo => grupo.length),
											multiplicadores = (xs =>
												xs.map((_, i) =>
													(xs => xs.reduce((a, b) => a * b, 1))(xs.slice(0, i))
												))(quantidades);
										return {
											maximo: grupos
												.map(
								...
								for (const grupo of elementos)
									for (const { valor, elemento } of grupo)
										elemento.adicionarTexto(valor + '');
								return {
									maximo,
									setValor(valor) {
										if (valor > maximo)
											throw new RangeError(`Valor inválido: ${valor}.`);
										let restante = valor;
										for (const grupo of elementos) {
				...
				controladorDigitos = (maximo => {
					...
					return {
						pushDígito(digito) {
							...
							for (; valor > maximo; )
								valor = obterValor((array = array.slice(1)));
							return valor;
						},
					};
				})(selecionador.maximo);
			...
			document.addEventListener(
				'keypress',
				(({
					pushDígito: pushDígito,
					mostrarTexto,
					setValor,
					baixar,
				}) => evt => {
					const digito = (string =>
						/^\d$/.test(string) ? Number(string) : null)(evt.key);
					if (null !== digito) {
