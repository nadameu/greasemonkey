import { GM, GM_addStyle, unsafeWindow } from '$';
...
function eproc_analisar_tabela(tabela: HTMLTableElement) {
...
async function abrir_aba_pdpj(numero: string) {
...
function createBroadcastService<Msg>(
  id: string,
  isValidMsg: (x: unknown) => x is Msg
) {
  const listeners = new Set<(_: Msg) => void>();
  ...
  function subscribe(listener: (_: Msg) => void) {
...
function validar_mensagem(msg: unknown): msg is { processo_aberto: string } {
  return (
    typeof msg === 'object' &&
    msg !== null &&
