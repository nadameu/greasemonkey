...
function safeHTML(strings, ...vars) {
    return vars.reduce((result, variable, i) => result +
...
class GUI {
    constructor() {
        this.avisoCarregando = {
            acrescentar(qtd) {
                ...
                const atual = progresso.value, total = progresso.max;
        ...
        if (!head)
            throw new Error('Erro ao localizar o elemento "head".');
        head.appendChild(estilos);
    }
    atualizarVisualizacao(localizador, filtrado = false) {
        if (!localizador.infoLink)
            return;
        ...
        const aVencer = new Date(agora.getFullYear(), agora.getMonth(), agora.getDate() + DIAS_A_FRENTE, ULTIMA_HORA, ULTIMO_MINUTO, ULTIMO_SEGUNDO, ULTIMO_MILISSEGUNDO);
        ...
        const baloes = prioridades.map((processos, indicePrioridade) => `<span id="gmLocalizador${localizador.infoLink.id}Prioridade${indicePrioridade}" class="gmProcessos gmPrioridade${indicePrioridade}${processos.length > 0 ? '' : ' gmVazio'}" onmouseover="infraTooltipMostrar(&quot;${avisos[indicePrioridade]}&quot;);" onmouseout="infraTooltipOcultar();">${processos.length}</span>`);
        const conteudo = [];
        ...
        baloes.unshift(`<span id="gmLocalizador${localizador.infoLink.id}Peticoes" class="gmPeticoes${processosComPeticao.length > 0 ? '' : ' gmVazio'}" onmouseover="infraTooltipMostrar(&quot;Processos com localizador PETIÇÃO ou SUSPENSOS-RETORNO&quot;);" onmouseout="infraTooltipOcultar();">${processosComPeticao.length > OCULTAR_BALOES_COM_MAIS_DE ? '+' : processosComPeticao.length}</span>`);
        conteudo.push('<div class="gmBaloes">');
        conteudo.push(baloes.join(''));
        conteudo.push('</div>');
        if (linha.cells.length < 1)
            throw new Error('Linha não possui espaço para escrita.');
        ...
        if (localizador.quantidadeProcessosNaoFiltrados > 0) {
            ...
            if (filtrado) {
                ...
                filtrar.addEventListener('click', async (evt) => {
                    evt.preventDefault();
                    evt.stopPropagation();
            ...
            atualizar.addEventListener('click', async (evt) => {
                evt.preventDefault();
                evt.stopPropagation();
                Array.from(document.getElementsByClassName('gmDetalhesAberto')).forEach(balaoAberto => {
                    const linhaAberta = balaoAberto.closest('tr');
            ...
            if (!divExistente)
                throw new Error('DIV inexistente.');
        ...
        function alternarDetalhes(balao, processos, indicePrioridade) {
            ...
            const MENOR = -1, IGUAL = 0, MAIOR = +1;
            processos.sort((a, b) => {
                if (a.termoPrazoCorregedoria < b.termoPrazoCorregedoria)
                    return MENOR;
                if (a.termoPrazoCorregedoria > b.termoPrazoCorregedoria)
                    return MAIOR;
                return IGUAL;
            ...
            processos.forEach((processo, indiceProcesso) => {
                ...
                const DIGITOS_CLASSE = 6, DIGITOS_COMPETENCIA = 2;
                ...
                const MAXIMO_PRIORIDADE_MENOR_OU_IGUAL_A_UM = 2, MAXIMO_PRIORIDADE_DOIS_OU_MAIOR = 1, DIAS_BAIXO = 3, IDEAL = 0.5;
                ...
                linhaNova.innerHTML = [
                    '<td>',
                    safeHTML `<img class="gmLembreteProcesso${processo.lembretes.length === 0 ? ' gmLembreteProcessoVazio' : ''}" src="infra_css/imagens/balao.gif" onmouseover="return infraTooltipMostrar('${processo.lembretes
                        .map(lembrete => lembrete.replace(/\n/g, '<br>'))
                        .join('<hr style="border-width: 0 0 1px 0;">')}', 'Lembretes', 400);" onmouseout="return infraTooltipOcultar();">`,
                    [
                        `<a href="${processo.link}">${processo.numprocFormatado}</a>`,
                        `<abbr title="${textoData}">${processo[processo.campoDataConsiderada]
                            .toLocaleString()
                            .substr(0, 10)}</abbr> + ${esperado.toString().replace(/\.5$/, '&half;')}${esperado >= 2 ? ' dias' : ' dia'} = ${processo.termoPrazoCorregedoria.toLocaleString().substr(0, 10)}`,
                    ].join(' | '),
        ...
        prioridades.forEach((processos, indicePrioridade) => {
            const balao = document.getElementById(`gmLocalizador${localizador.infoLink ? localizador.infoLink.id : '0'}Prioridade${indicePrioridade}`);
            balao === null || balao === void 0 ? void 0 : balao.addEventListener('click', evt => {
                evt.preventDefault();
                evt.stopPropagation();
                alternarDetalhes(balao, processos, indicePrioridade);
            }, false);
        });
        const balaoPeticoes = document.getElementById(`gmLocalizador${localizador.infoLink.id}Peticoes`);
        balaoPeticoes === null || balaoPeticoes === void 0 ? void 0 : balaoPeticoes.addEventListener('click', function (evt) {
            evt.preventDefault();
            evt.stopPropagation();
    ...
    criarBotaoAcao(localizadores) {
        ...
        function criarCheckboxMostrar(id, padrao, [classeTrue, classeFalse], texto) {
            ...
            function onChange() {
                ...
                if (input.checked) {
                    if (classeFalse !== '')
                        document.body.classList.remove(classeFalse);
                    if (classeTrue !== '')
                        document.body.classList.add(classeTrue);
                }
                else {
                    if (classeTrue !== '')
                        document.body.classList.remove(classeTrue);
                    if (classeFalse !== '')
                        document.body.classList.add(classeFalse);
                }
            }
        ...
        const labelClasses = criarCheckboxMostrar('mostrarClasses', true, ['', 'gmNaoMostrarClasses'], ' Mostrar classe dos processos');
        frag.appendChild(labelClasses);
        frag.appendChild(document.createElement('br'));
        const labelDias = criarCheckboxMostrar('mostrarDiasParaFim', true, ['', 'gmNaoMostrarDiasParaFim'], ' Mostrar dias para o fim do prazo');
        frag.appendChild(labelDias);
        frag.appendChild(document.createElement('br'));
        const labelConsiderarDataInclusaoLocalizador = criarCheckboxMostrar('considerarDataInclusaoLocalizador', false, ['gmConsiderarDataInclusaoLocalizador', ''], 'Entre data do último evento e da inclusão no localizador, considerar a mais antiga');
        frag.appendChild(labelConsiderarDataInclusaoLocalizador);
        frag.appendChild(document.createElement('br'));
        const labelPrazoMetade = criarCheckboxMostrar('prazoMetade', true, ['gmPrazoMetade', ''], 'Conceder metade do prazo normal para processos prioritários');
    ...
    atualizarTabelaExtra(localizadores) {
        ...
        if (!tabela) {
            ...
            (_c = (_b = (_a = localizadores[0]) === null || _a === void 0 ? void 0 : _a.linha.closest('table')) === null || _b === void 0 ? void 0 : _b.parentNode) === null || _c === void 0 ? void 0 : _c.appendChild(tabela);
            tabela.createTBody();
        }
        ...
        const info = localizadores
            ...
            .sort((a, b) => b.valor - a.valor);
        (_e = (_d = localizadores[0]) === null || _d === void 0 ? void 0 : _d.linha.parentNode) === null || _e === void 0 ? void 0 : _e.querySelectorAll('.gmDetalhes').forEach(x => x.remove());
        (_g = (_f = localizadores[0]) === null || _f === void 0 ? void 0 : _f.linha.parentNode) === null || _g === void 0 ? void 0 : _g.append(...info.map(loc => loc.linha));
        const temp = localizadores.map(localizador => localizador.processos.map(processo => ({ processo, localizador })));
        const sep = []
            .concat(...temp)
        ...
        for (const { processo: proc, localizador } of sep) {
            ...
            html += '</td>';
            html += '<td class="infraTd">';
            html += new Intl.NumberFormat('pt-BR', { style: 'percent' }).format(proc.atrasoPorcentagem + 1);
            html += '</td>';
            row.innerHTML = html;
    ...
    criarGrafico(localizadores) {
        ...
        function extrairProcessos(localizadores) {
            ...
            localizadores.forEach(localizador => {
                localizador.processos.forEach(processo => {
                    ...
                    const termo = processo.termoPrazoCorregedoria, dataTermo = new Date(termo.getFullYear(), termo.getMonth(), termo.getDate());
                    const timestamp = Math.max(hoje.getTime(), dataTermo.getTime());
                    if (processos.has(numproc)) {
                        const timestampAntigo = processos.get(numproc), timestampNovo = Math.min(timestampAntigo, timestamp);
        ...
        class Grafico {
            constructor() {
                this.dimensoes = {
                    get largura() {
                        ...
                        if (!areaTela)
                            throw new Error('Elemento "#divInfraAreaTelaD" não encontrado.');
            ...
            get area() {
                ...
                const margemExterna = this.dimensoes.margem +
            ...
            get dadosTrintaDias() {
                const UM_DIA = 864e5;
                const agora = new Date(), hoje = new Date(agora.getFullYear(), agora.getMonth(), agora.getDate()).getTime(), maximo = hoje + 30 * UM_DIA;
            ...
            desenharEscala() {
                const context = this.context;
                const xTexto = this.dimensoes.margem +
                    this.linha.espessura / 2 +
                    this.dimensoes.espacamento +
            ...
            calcularCategorias() {
                ...
                const agora = new Date(), hoje = new Date(agora.getFullYear(), agora.getMonth(), agora.getDate()).getTime(), maximo = Math.max.apply(null, dias);
            ...
            assegurarDistanciaMinima(unidade, distancia) {
                ...
                [1, 2, 2.5, 5, 10].forEach(mult => {
                    if (tamanhoIdealEncontrado)
                        return;
                    let novoIntervalo = this.escala[unidade] * mult;
                    if (novoIntervalo % 1 !== 0)
                        return;
        ...
        if (!areaTela)
            throw new Error('Não foi possível encontrar o elemento "#divInfraAreaTelaD".');
...
const obterFormularioRelatorioGeral = memoize(async () => {
    ...
    if (!url)
        throw new Error('Não foi possível obter o link para o relatório geral.');
    ...
    if (!form)
        throw new Error('Formulário do relatório geral não encontrado.');
    return form;
});
async function trataHTML(localizador, doc) {
    var _a, _b, _c;
    const pagina = Number((_a = doc.querySelector('input#hdnInfraPaginaAtual')) === null || _a === void 0 ? void 0 : _a.value);
    if (isNaN(pagina))
        throw new Error('Não foi possível obter a página.');
    const quantidadeProcessosCarregados = parseInt((_c = (_b = doc.querySelector('input#hdnInfraNroItens')) === null || _b === void 0 ? void 0 : _b.value) !== null && _c !== void 0 ? _c : '');
    ...
    if (todas) {
        ...
        await Promise.all(Array.from({ length: todas.options.length - 1 }, (_, i) => i + 1).map(p => localizador.obterPagina(p, doc)));
        return localizador;
    }
...
class Localizador {
    ...
    get quantidadeProcessos() {
        var _a, _b;
        return Number((_b = (_a = this.infoLink) === null || _a === void 0 ? void 0 : _a.link.textContent) !== null && _b !== void 0 ? _b : '0');
    }
    async obterPagina(pagina, doc) {
        try {
            let url, data;
            if (!this.infoLink)
                throw new Error('Não foi possível obter o endereço da página do localizador "' + this.nomeExibicao + '".');
            if (pagina === 0) {
                ...
                if (!select)
                    throw new Error('Não foi possível obter o seletor de página.');
                select.value = this.infoLink.id;
                const paginaAtual = doc.querySelector('input#hdnInfraPaginaAtual');
                if (!paginaAtual)
                    throw new Error('Não foi possível localizar a página atual.');
    ...
    async obterPrazosPagina(pagina = 0) {
        ...
        const quantidadeProcessosCarregados = parseInt((_b = (_a = doc.querySelector('input#hdnInfraNroItens')) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : '');
        if (isNaN(quantidadeProcessosCarregados))
            throw new Error('Não foi possível obter a quantidade de processos analisados.');
        if (tabela) {
            console.log(pagina, this.infoNome, (_c = tabela.querySelector('caption')) === null || _c === void 0 ? void 0 : _c.textContent);
            ...
            linhas.forEach(linha => {
                var _a;
                const link = (_a = linha.cells[1]) === null || _a === void 0 ? void 0 : _a.querySelector('a[href]');
                if (!link)
                    throw new Error('Link não encontrado.');
                const numproc = new URL(link.href).searchParams.get('num_processo');
                processosComPrazoAberto.add(numproc);
            });
            this.processos = this.processos.filter(processo => !processosComPrazoAberto.has(processo.numproc));
        }
        else {
            console.log(pagina, this.infoNome, quantidadeProcessosCarregados);
        }
        if (doc.getElementById('lnkProximaPaginaSuperior')) {
            const paginaAtual = parseInt((_e = (_d = doc.querySelector('select#selInfraPaginacaoSuperior')) === null || _d === void 0 ? void 0 : _d.value) !== null && _e !== void 0 ? _e : '');
            if (isNaN(paginaAtual))
                throw new Error('Não foi possível detectar a página atual.');
        ...
        if (isNaN(qtd))
            throw new Error('Erro ao obter a quantidade de processos.');
    ...
    async obterProcessos() {
        ...
        if (!infoLink)
            throw new Error('Localizador não possui identificador');
        await this.obterPagina(0);
        this.quantidadeProcessosNaoFiltrados = this.processos.length;
        link.textContent = String(this.processos.length);
        if (this.processos.length > 0) {
            const localizadorProcesso = this.processos[0].localizadores.filter(localizador => localizador.id === infoLink.id)[0];
...
class LocalizadorFactory {
    static fromLinha(linha) {
        var _a, _b, _c, _d;
        const separador = ' - ';
        const siglaNome = ((_b = (_a = linha.cells[0]) === null || _a === void 0 ? void 0 : _a.textContent) === null || _b === void 0 ? void 0 : _b.trim()) || '';
        ...
        if (!link)
            throw new Error(`Link para a lista de processos não encontrada: "${siglaNome}".`);
        const match = (((_c = link.textContent) === null || _c === void 0 ? void 0 : _c.trim()) || '').match(/^\d+$/);
        if (!match)
            throw new Error(`Quantidade de processos não encontrada: "${siglaNome}".`);
    ...
    static fromLinhaPainel(linha) {
        var _a, _b, _c, _d, _e;
        const nome = (_c = (_b = (_a = linha.cells[0]) === null || _a === void 0 ? void 0 : _a.textContent) === null || _b === void 0 ? void 0 : _b.match(/^Processos com Localizador\s+"(.*)"$/)) === null || _c === void 0 ? void 0 : _c[1];
        if (!nome)
            throw new Error('Nome do localizador não encontrado.');
        const link = linha.querySelector('a,u');
        if (!link)
            throw new Error('Link não encontrado.');
        const quantidadeProcessosNaoFiltrados = parseInt((_d = link.textContent) !== null && _d !== void 0 ? _d : '');
        let id;
...
class LocalizadorProcessoFactory {
    static fromInput(input) {
        ...
        if (!elementoNome)
            throw new Error('Não foi possível obter o nome do localizador.');
        const principal = elementoNome.nodeName.toLowerCase() === 'u';
        const sigla = (_b = (_a = elementoNome.textContent) === null || _a === void 0 ? void 0 : _a.trim()) !== null && _b !== void 0 ? _b : '';
        if (!sigla)
            throw new Error('Localizador não possui sigla.');
        const linkLembrete = elementoNome.nextElementSibling;
        let lembrete;
        if (linkLembrete === null || linkLembrete === void 0 ? void 0 : linkLembrete.attributes.hasOwnProperty('onmouseover')) {
            const onmouseover = (_d = (_c = linkLembrete.attributes.getNamedItem('onmouseover')) === null || _c === void 0 ? void 0 : _c.value) !== null && _d !== void 0 ? _d : '';
            lembrete =
                (_f = (_e = onmouseover.match(/^return infraTooltipMostrar\('Obs: (.*) \/ ([^(]+)\(([^)]+)\)','',400\);$/)) === null || _e === void 0 ? void 0 : _e[1]) !== null && _f !== void 0 ? _f : undefined;
        }
...
class LocalizadoresProcessoFactory {
    static fromCelula(celula) {
        const localizadoresProcesso = [...celula.getElementsByTagName('input')].map(LocalizadorProcessoFactory.fromInput);
...
const MILISSEGUNDOS_EM_UM_DIA = 864e5;
const COMPETENCIA_JUIZADO_MIN = 9, COMPETENCIA_JUIZADO_MAX = 20, COMPETENCIA_CRIMINAL_MIN = 21, COMPETENCIA_CRIMINAL_MAX = 30, COMPETENCIA_EF_MIN = 41, COMPETENCIA_EF_MAX = 43, CLASSE_EF = 99, CLASSE_CARTA_PRECATORIA = 60;
const DOMINGO = 0, SEGUNDA = 1, TERCA = 2, QUARTA = 3, QUINTA = 4, SEXTA = 5, SABADO = 6;
...
const JANEIRO = 0, MAIO = 4, DEZEMBRO = 11;
...
function calcularAtraso(a, b) {
    ...
    let proximaSuspensao = recesso.inicio < inspecao.inicio
...
class Processo {
    constructor({ classe, dadosComplementares, dataAutuacao, dataInclusaoLocalizador, dataSituacao, dataUltimoEvento, juizo, lembretes, linha, link, localizadores, numClasse, numCompetencia, numproc, numprocFormatado, sigilo, situacao, }) {
    ...
    get competenciaCorregedoria() {
        if (this.numCompetencia >= COMPETENCIA_JUIZADO_MIN &&
            this.numCompetencia <= COMPETENCIA_JUIZADO_MAX) {
            return CompetenciasCorregedoria.JUIZADO;
        }
        else if (this.numCompetencia >= COMPETENCIA_CRIMINAL_MIN &&
            this.numCompetencia <= COMPETENCIA_CRIMINAL_MAX) {
            return CompetenciasCorregedoria.CRIMINAL;
        }
        else if ((this.numCompetencia >= COMPETENCIA_EF_MIN || this.numCompetencia <= COMPETENCIA_EF_MAX) &&
    ...
    get campoDataConsiderada() {
        ...
        if (this._campoDataConsiderada === undefined) {
            const infos = (_a = this.localizadores
                .map(loc => {
                var _a, _b, _c, _d, _e, _f;
                return ((_f = (_c = (_b = (_a = infoMeta[loc.id]) === null || _a === void 0 ? void 0 : _a[this.situacao]) === null || _b === void 0 ? void 0 : _b.campoDataConsiderada) !== null && _c !== void 0 ? _c : (_e = (_d = infoMeta[loc.id]) === null || _d === void 0 ? void 0 : _d['*']) === null || _e === void 0 ? void 0 : _e.campoDataConsiderada) !== null && _f !== void 0 ? _f : null);
            })
                ...
                : curr === null
                    ...
                    : this[curr] < this[prev]
                        ? curr
                        : prev)) !== null && _a !== void 0 ? _a : 'dataSituacao';
    ...
    get prazoCorregedoria() {
        ...
        if (this._prazoCorregedoria === undefined) {
            const infos = (_a = this.localizadores
                ...
                return ((_h = (_d = (_c = (_b = (_a = infoMeta[loc.id]) === null || _a === void 0 ? void 0 : _a[this.situacao]) === null || _b === void 0 ? void 0 : _b.dias) === null || _c === void 0 ? void 0 : _c[this.competenciaCorregedoria]) !== null && _d !== void 0 ? _d : (_g = (_f = (_e = infoMeta[loc.id]) === null || _e === void 0 ? void 0 : _e['*']) === null || _f === void 0 ? void 0 : _f.dias) === null || _g === void 0 ? void 0 : _g[this.competenciaCorregedoria]) !== null && _h !== void 0 ? _h : null);
            })
                .reduce((prev, curr) => prev === null
                ? curr === null
                    ? null
                ...
                : curr === null
                    ? prev
                    : Math.min(prev, curr))) !== null && _a !== void 0 ? _a : 1;
    ...
    get termoPrazoCorregedoria() {
        if (this._termoPrazoCorregedoria === null) {
            ...
            if (data >= proximaSuspensao.inicio)
...
class ProcessoFactory {
    static fromLinha(linha) {
        ...
        if (!link)
            throw new Error('Link para o processo não encontrado.');
        ...
        let lembretes = [];
        if (links.length === 2) {
            const onmouseover = (_b = (_a = [...links[1].attributes].filter(attr => attr.name === 'onmouseover')[0]) === null || _a === void 0 ? void 0 : _a.value) !== null && _b !== void 0 ? _b : '';
            const match = onmouseover.match(/^return infraTooltipMostrar\('([^']+)','Lembretes',400\);$/);
            if (!match)
                throw new Error('Não foi possível obter lembretes do processo.');
        ...
        const textoSigilo = (_d = (_c = linha.cells[1].getElementsByTagName('br')[0]) === null || _c === void 0 ? void 0 : _c.nextSibling) === null || _d === void 0 ? void 0 : _d.textContent;
        const sigilo = Number((_e = textoSigilo === null || textoSigilo === void 0 ? void 0 : textoSigilo.match(/Nível ([0-5])/)) === null || _e === void 0 ? void 0 : _e[1]);
        if (isNaN(sigilo))
            throw new Error('Não foi possível obter o nível de sigilo do processo.');
        ...
        const dataUltimoEvento = parseDataHora((_f = breakUltimoEvento === null || breakUltimoEvento === void 0 ? void 0 : breakUltimoEvento.previousSibling) === null || _f === void 0 ? void 0 : _f.textContent);
        const ultimoEvento = (_g = breakUltimoEvento === null || breakUltimoEvento === void 0 ? void 0 : breakUltimoEvento.nextSibling) === null || _g === void 0 ? void 0 : _g.textContent;
...
function main() {
    ...
    if (/\?acao=usuario_tipo_monitoramento_localizador_listar&/.test(location.search)) {
        let tabela = (_a = document.getElementById('divInfraAreaTabela')) === null || _a === void 0 ? void 0 : _a.querySelector('table');
        if (!tabela)
            throw new Error('Não foi possível obter a tabela de localizadores.');
        ...
        let tabela = (_b = document.getElementById('fldLocalizadores')) === null || _b === void 0 ? void 0 : _b.querySelector('table');
        if (!tabela)
            throw new Error('Não foi possível obter a tabela de localizadores.');
...
function parsePares(pares) {
    return pares.reduce((obj, par) => {
        ...
        if (dividido.length < 2)
            throw new Error(`Não foi possível analisar o texto "${par}".`);
