	...
	atualizarResultadoBeneficiario().then(valoresCalculados => {
		criarBotaoApos(resultado, 'Usar valores calculados', evt => {
			...
			if (Propriedades.haCorrente || Propriedades.haAnterior) {
				deducao.value = 'S';
				corrente.value = Propriedades.haCorrente ? Moeda.formatar(valoresCalculados.correnteAutor) : '';
				anterior.value = Propriedades.haAnterior ? Moeda.formatar(valoresCalculados.anteriorAutor) : '';
			} else {
				deducao.value = 'N';
			}
			deducao.onchange();
...
function adicionarAlteracoesHonorario() {
	...
	const botao = criarBotaoApos(resultado, 'Usar valores calculados', evt => {
		...
		const juros = document.querySelector('[name="val_juros"]');
		const total = document.querySelector('[name="val_principal_com_juros"]');
		if (tipo.value === '2#O') { // Honorários de sucumbência
		...
		} else if (tipo.value === '53#O') { // Honorários contratuais
...
function adicionarAlteracoesRequisicao() {
	...
	div.innerHTML = `
<label class="infraLabelOpcional" for="gm-rpv__principal">Valor principal corrigido: </label><input id="gm-rpv__principal" class="gm-rpv__principal" oninput="formatarCampoMoeda(this);" value="${Moeda.formatar(Propriedades.principal)}"><br>
<label class="infraLabelOpcional" for="gm-rpv__total">Valor total (principal + juros): </label><input id="gm-rpv__total" class="gm-rpv__total" oninput="formatarCampoMoeda(this);" value="${Moeda.formatar(Propriedades.total)}"><br>
<br>
<div class="gm-rpv__corrente__container"><input type="checkbox" id="gm-rpv__corrente"${Propriedades.haCorrente ? ' checked' : ''}><label class="infraLabelOpcional" for="gm-rpv__corrente">Exercício corrente - valor total (principal + juros): </label><input class="gm-rpv__corrente__valor" oninput="formatarCampoMoeda(this);" value="${Moeda.formatar(Propriedades.corrente)}"${Propriedades.haCorrente ? '' : ' disabled'}><br></div>
<input type="checkbox" id="gm-rpv__anterior"${Propriedades.haAnterior ? ' checked' : ''}><label class="infraLabelOpcional" for="gm-rpv__anterior">Exercício anterior - valor total (principal + juros): </label><input class="gm-rpv__anterior__valor" oninput="formatarCampoMoeda(this);" value="${Moeda.formatar(Propriedades.anterior)}"${Propriedades.haAnterior ? '' : ' disabled'}><br>
<br>
<label class="infraLabelOpcional" for="gm-rpv__honorarios-contratuais">Honorários contratuais: </label><input id="gm-rpv__honorarios-contratuais" class="gm-rpv__honorarios-contratuais" type="number" step="${STEPS_HONORARIOS}" value="${Propriedades.honorarios}" min="0" max="100"> %<br>
<br>
<label class="infraLabelOpcional" for="gm-rpv__honorarios-sucumbencia">Honorários de sucumbência: </label><input id="gm-rpv__honorarios-sucumbencia" class="gm-rpv__honorarios-sucumbencia" oninput="formatarCampoMoeda(this);" value="${Moeda.formatar(Propriedades.sucumbencia)}"><br>
<br>
<label class="infraLabelOpcional" for="gm-rpv__outros">Outros: </label><input id="gm-rpv__outros" class="gm-rpv__outros" oninput="formatarCampoMoeda(this);" value="${Moeda.formatar(Propriedades.outros)}"><br>
	`;
	labelTotal.parentElement.insertBefore(div, labelTotal);

	let valoresCalculados;
	function atualizarResultado() {
		return atualizarResultadoRequisicao().then(valores => {
			valoresCalculados = valores;
			botao.style.display = '';
		}).catch(() => {
			botao.style.display = 'none';
...
function atualizarResultadoBeneficiario() {
	return mostrarSeNaoHouverErros(valoresCalculados => gerarHTMLBeneficiario(valoresCalculados, true));
}
...
function calcularValores() {
	...
	if ((haCorrente || haAnterior) && arredondarMoeda(somaIRPF) !== total) {
		...
		erros.push(`${inicio} (${Moeda.formatar(somaIRPF)}) não corresponde ao total (${Moeda.formatar(total)}).`);
	}
	if (erros.length > 0) {
