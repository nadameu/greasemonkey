...
function matchWith(resultado, matchers) {
    if (resultado.isValido)
        return matchers.Ok(resultado.valor);
...
function traverse(xs, f) {
    return xs.reduce((fys, x) => lift2(fys, f(x), (ys, y) => {
        ys.push(y);
        return ys;
    }), Ok([]));
}
function traverseObj(obj, f) {
    return Object.entries(obj).reduce((resultado, [key, x]) => lift2(resultado, f(x), (target, y) => {
        target[key] = y;
        return target;
    }), Ok({}));
}
...
function main() {
    ...
    const resTotal = map(resTerminativos, ({ eventos, sentenca, autores }) => {
        const { verificarTransito, verificarCumprimentoObricacaoFazer, verificarPagamentoAutor, verificarPagamentoPerito, } = comEventos(eventos);
...
function verificarLocalizadores() {
    ...
    if (localizadores.length !== 1)
        return Invalido(['Mais de um localizador.']);
...
function verificarAutores() {
    ...
    if (autores.length < 1)
        return Invalido(['Não há autores.']);
    return Ok(autores);
}
function verificarSentenca(eventos) {
    const sentenca = eventos.find(({ descricao, memos }) => descricao.match(/Sentença com Resolução de Mérito - Conciliação\/Transação Homologada /) &&
        ...
        eventos.find(({ descricao, memos }) => descricao.match(/Sentença com Resolução de Mérito - Pedido Procedente/) &&
            memos.match(/ACOLHO(,?\s+em\s+parte,?)?\s+os?\s+pedidos?.*condenar\s+o\s+INSS/i));
    if (!sentenca)
        return Invalido(['Não há sentença de acordo ou procedência.']);
    return Ok(sentenca);
}
function comEventos(eventos) {
    ...
    function verificarCumprimentoObricacaoFazer(sentenca) {
        const intimacaoAPSSentenca = eventos.find(({ descricao, referente, ordinal }) => descricao.match(/Intimação Eletrônica - Expedida\/Certificada - Requisição/) &&
        ...
        if (!intimacaoAPSSentenca)
            return Invalido(['APSADJ não foi intimada da sentença.']);
        const eventosAPS = eventos.filter(({ aps }) => aps);
        const respostaOriginal = eventosAPS.find(({ referente }) => referente.some(ref => ref === intimacaoAPSSentenca.ordinal));
        ...
        if (!ultimaResposta)
            return Invalido(['APSADJ não juntou resposta.']);
        const intimacaoAutorResposta = eventos.find(({ descricao, referente }) => descricao.match(/Intimação Eletrônica - Expedida\/Certificada/) &&
    ...
    function verificarPagamentoAutor(autor) {
        const pagamento = eventos.find(({ descricao }) => descricao.match(new RegExp(`Requisição de Pagamento - Pequeno Valor - Paga - Liberada .*\\(${autor}\\)`, 'i')));
        if (!pagamento)
            return Invalido([`Não houve pagamento do autor ${autor}.`]);
        ...
        const intimacao = eventos.find(({ descricao, referente }) => descricao.match(/Intimação Eletrônica - Expedida\/Certificada/) &&
    ...
    function verificarPagamentoPerito(perito) {
        const pagamento = eventos.find(({ descricao }) => descricao.match(new RegExp(`Requisição de Pagamento - Pequeno Valor - Paga - Liberada .*\\(${perito}\\)`, 'i')));
        if (!pagamento)
...
function baixarMotivo(motivo) {
    inserirAntesDaCapa(`<div style="display: inline-block; padding: 4px; border-radius: 4px; font-size: 1.25em; font-weight: bold; background: #848; color: #fff;">Baixar motivo ${motivo}</div>`);
}
function rejeitarMotivo(motivo) {
    inserirAntesDaCapa(`<div style="display: inline-block; padding: 4px; border-radius: 4px; font-size: 1.25em; font-weight: regular; background: #c72; color: #fff;">${motivo}</div>`);
}
...
function houveDecursoOuCiencia(eventos, ordinal) {
    return eventos
        ...
        .find(({ descricao, referente }) => (descricao.match(/Decurso de Prazo/) || descricao.match(/RENÚNCIA AO PRAZO/)) &&
...
function parseEventos(eventos) {
    return eventos.map(linha => {
        ...
        const aps = safePipe(linha.cells[4], c => c.querySelector('label'), l => l.getAttribute('onmouseover'), a => a.match('AG. PREV. SOCIAL')) != null;
