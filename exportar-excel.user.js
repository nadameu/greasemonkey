...
function main() {
  const eventos = Array
    .from(document.querySelectorAll('#tblEventos > tbody > tr[class^="infraTr"]'))
    .map(linha => ({
      linha,
...
function obterDados(eventos) {
  ...
  const numProcesso = safePipe(
    numProcessoFormatado,
    x => x.replace(/[.-]/g, ''),
  );

  const autores = Array
    .from(document.querySelectorAll('.infraNomeParte[data-parte="AUTOR"]'))
    .map(x => x.textContent);

  const reus = Array
    .from(document.querySelectorAll('.infraNomeParte[data-parte="REU"]'))
    .map(x => x.textContent);
